...
**左键点托盘图标**，弹出一个独立的卡片面板，用进度条画出各窗口用量，显示按量付费的
「额外用量」额度，底部还有「立即刷新 / 退出」两个按钮（点面板外面任意处即收起）：

<p align="center">
  <img src="assets/image.png" alt="左键弹出的用量面板" width="320">
</p>

进度条颜色与托盘图标同源（绿→黄→橙→红）。面板**按显示器真实 DPI 绘制**，在 100% / 125% /
150% / 200% 等常见缩放下都清晰不发糊。**右键点托盘图标**只剩一个「退出」兜底项（用量明细已
全部搬进左键面板）。

- **默认每 60 秒自动刷新**（可在配置文件里改间隔），也可点面板上的「立即刷新」手动拉取。
...
双击 `claude-usage-widget.exe`，托盘区立刻出现仪表图标。**左键**弹用量面板（含「立即刷新 /
退出」按钮），**右键**只有一个「退出」兜底项。

> 改了代码重新构建后，记得**先退出旧图标再启动新 exe**——否则托盘里还是旧进程，看不到变化。

### 4. 开机自启（可选）

把快捷方式丢进启动文件夹即可（widget 本身不碰注册表、不留任何自启残留）：
...
首次运行会在 `~/.claude/claude-usage-widget.json` 写出一份默认配置，按需编辑后**重启 widget**
生效：

```jsonc
{
  "poll_interval_seconds": 60,        // 轮询间隔（秒），下限 10
  "icon_source": "five_hour"          // 托盘图标按哪个窗口着色："five_hour" 或 "seven_day"
}
```

文件缺失或字段非法时一律退回内置默认值，不影响主功能。
...
2. **阈值桌面通知** —— 用量超过阈值（如 90%）时弹一条 Windows toast。
3. **macOS 菜单栏版** —— 复用 `internal/auth`、`internal/usage` 核心逻辑，仅换 UI 层。
4. **单元测试 + CI** —— 补纯函数测试，GitHub Actions 自动构建/发布多架构 release。

> ✅ 已完成：**动态仪表图标**（弧长按真实百分比绘制）、exe 与托盘图标风格统一、低开销调优、
> **配置文件**（轮询间隔/图标取色窗口）、**独立 Win32 用量面板**（进度条 + 额外用量额度 +
> 「立即刷新/退出」按钮，左键弹出，惰性创建、隐藏时零开销，按 DPI 缩放在各缩放比下都清晰）。
